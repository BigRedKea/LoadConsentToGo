...
namespace LoadConsentToGo
{
    ...
    {
        public static int Upsert(string dbfilepath, List<C2GDownload> c2gdata)
        {
            ...
            using (var connection = new SqliteConnection(connectionString))
            {
                ...
                string createTableSql = @"
                    CREATE TABLE IF NOT EXISTS Consent2GoProfiles (
                ...
                foreach (var c2g in c2gdata)
                {
                    ...
                    if (existingdata.Count(x => x.UniqueIdentifier == c2g.UniqueIdentifier) == 0)
                    {
            ...
            static string? SafeGetStringByName(SqliteDataReader r, string name)
            {
                int i = r.GetOrdinal(name); // cheap if you cache name->ordinal in production
                return r.IsDBNull(i) ? null : r.GetString(i);
            }


            void AddParameter(SqliteCommand command, string name, object? obj)
            {
